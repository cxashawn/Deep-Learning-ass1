...
trainData = {
    data = Data.data[{{1,trsize}}],
    labels = Data.labels[{{1,trsize}}],
    size = function() return trsize end
}

testData = {
   data = Data.data[{{trsize + 1,trsize + 1 + tesize}}],
   labels = Data.labels[{{trsize + 1,trsize + 1 + tesize}}],
...
if opt.visualize then
   if itorch then
      ...
      t256Samples = testData.data[{ {1,256} }]
      itorch.image(t256Samples)
   else
